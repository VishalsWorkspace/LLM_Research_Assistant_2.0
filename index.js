// This file contains the main React App component and renders it into the HTML.
// React, useState, useEffect, useRef are available globally via CDN scripts.

// Main App component for the Offline LLM-Powered PDF Assistant
const App = () => {
    // State variables for managing UI elements and data
    const [query, setQuery] = React.useState(''); // Stores the user's input query
    const [response, setResponse] = React.useState(''); // Stores the LLM's response
    const [loading, setLoading] = React.useState(false); // Indicates if a query is being processed
    const [metrics, setMetrics] = React.useState(null); // Stores LLM performance metrics
    const [pdfFile, setPdfFile] = React.useState(null); // Stores the selected PDF file
    const [showUploadModal, setShowUploadModal] = React.useState(false); // Controls visibility of PDF upload modal
    const [recentPdfs, setRecentPdfs] = React.useState([]); // Stores a list of recently used PDFs (mock data for now)
    const [message, setMessage] = React.useState({ text: '', type: '' }); // For displaying success/error messages
